#!/usr/bin/env python3
"""
Insert restaurant test data with unique identifier for easy validation and cleanup.

Database schema reference:
restaurant_transaction_line columns: transaction_line_id, line_item_amount,
employee_discount_amount, tax_amount, tender_amount, restaurant_tip_amount,
department_id, class_id (all others nullable and omitted)
"""
...
def insert_test_data(count: int):
    ...
    try:
        print(f"Inserting {count} test restaurant transactions with store_of_intent={TEST_STORE}")
        ...
        conn.commit()
        print(f"Inserted {count} restaurant transactions successfully")
