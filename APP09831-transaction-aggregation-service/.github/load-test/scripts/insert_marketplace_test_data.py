#!/usr/bin/env python3
"""
Insert marketplace test data with unique identifier for easy validation and cleanup.
"""
...
def insert_test_data(count: int):
    ...
    try:
        print(f"Inserting {count} test marketplace transactions with store_of_intent={TEST_STORE}")
        ...
        for i in range(count):
            ...
            """, (transaction_id, f'{i}', 'MARKETPLACE', 'SALE', TEST_STORE))

            transaction_line_id = cursor.fetchone()[0]
            inserted_ids['transaction_lines'].append(transaction_line_id)

            # Insert marketplace transaction line
            cursor.execute("""
                INSERT INTO marketplace_transaction_line (
                    transaction_line_id,
                    line_item_amount, tax_amount,
                    tender_amount, marketplace_jwn_commission_amount
                ) VALUES (%s, %s, %s, %s, %s)
            """, (
                transaction_line_id,
                100.00,
        ...
        conn.commit()
        print(f"Inserted {count} marketplace transactions successfully")
