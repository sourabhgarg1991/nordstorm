#!/usr/bin/env python3
"""
Validates that retail transaction data from ERTM was correctly processed and persisted to Aurora DB.
...
3. RetailTransactionLine records were created
...
def validate_database_state(test_start_time):
    """
    Validates that Retail data was correctly processed to Aurora DB.
    ...
    print("=" * 60)
    print("ERTM CONSUMER LOAD TEST - DATABASE VALIDATION")
    print("=" * 60)
    ...
    try:
        ...
        if transaction_count == 0:
            ...
            print("  Expected: Transactions with source_reference_system_type='ertm' and source_reference_type='retail'")
            return False
        ...
        # Step 5: Get transaction_line IDs for ertm validation
        cursor.execute("""
        ...
        # Step 6: Count RetailTransactionLine records
        cursor.execute("""
            SELECT COUNT(*) 
            FROM retail_transaction_line
            WHERE transaction_line_id = ANY(%s)
        """, (transaction_line_ids,))

        retail_line_count = cursor.fetchone()[0]
        print(f"[PASS] RetailTransactionLine records created: {retail_line_count}")

        if retail_line_count == 0:
            print("ERROR: No retail transaction lines were created!")
            print("  Expected: RetailTransactionLine ")
            return False
        ...
        # Step 8: Verify retail lines integrity
        cursor.execute("""
            SELECT COUNT(*) 
            FROM retail_transaction_line ptl
        ...
        orphaned_retail_transactions = cursor.fetchone()[0]

        if orphaned_retail_transactions > 0:
            print(f"ERROR: Found {orphaned_retail_transactions} orphaned RetailTransactionLine records!")
            return False

        print("[PASS] No orphaned RetailTransactionLine records")
        print()
        ...
        cursor.execute("""
            SELECT 
                ...
                COUNT(ptl.retail_transaction_line_id) as retail_count
            FROM transaction t
            JOIN transaction_line tl ON t.transaction_id = tl.transaction_id
            JOIN retail_transaction_line ptl ON tl.transaction_line_id = ptl.transaction_line_id
        ...
        if sample:
            ...
            print(f"  Source System: {sample[2]} (expected: ertm)")
            print(f"  Source Type: {sample[3]} (expected: retail)")
            print(f"  Business Date: {sample[4]}")
            print(f"  Transaction Lines: {sample[5]}")
            print(f"  Retail Transaction Lines: {sample[6]}")
            print()
        ...
        print(f"[PASS] Retail Transaction Lines created: {retail_line_count}")
...
def main():
    parser = argparse.ArgumentParser(description='Validate retail data in Aurora DB')
